...
# 데이터 로드 실패 시 빈 리스트로 초기화하여 에러 방지
if not PROBLEM_SETS or all(not s for s in PROBLEM_SETS):
    st.warning("문제 데이터를 불러올 수 없습니다. problems.json 파일을 확인해주세요.")
    PROBLEM_SETS = [[], [], [], [], []]
...
def main():
    ...
    if 'selected_set_idx' not in st.session_state:
        # 비어있지 않은 세트 중에서만 선택
        valid_indices = [i for i, p_set in enumerate(PROBLEM_SETS) if p_set]
        if valid_indices:
            st.session_state.selected_set_idx = random.choice(valid_indices)
        else:
            st.session_state.selected_set_idx = 0
    ...
    # --- 페이지 2: 시험 진행 ---
        ...
        st.subheader(f"수험번호: {st.session_state.user_info.get('code', 'Unknown')}")
        st.markdown("---")
        ...
        with st.form("test_form"):
            ...
            for idx, q in enumerate(obj_questions):
                ...
                # index=None은 Streamlit 최신 버전 기능입니다. 구버전일 경우 에러가 날 수 있어 try 처리하지 않고 그대로 둡니다.
        ...
        with st.spinner("AI가 채점 및 분석 중입니다... (약 10~20초 소요)"):
            ...
            if user_writing:
                try:
                    ...
                    아래 학생의 쓰기 답안을 3~4급 수준을 기준으로 평가하고, 반드시 아래의 JSON 포맷으로만 출력하세요.
            ...
            doc_data = {
                "name_enc": st.session_state.user_info.get('name'),
                "univ_enc": st.session_state.user_info.get('code'),
            ...
            if user_writing:
                wa = writing_analysis
                breakdown = wa.get('breakdown', {"content":0, "structure":0, "grammar":0})
                st.write(f"**[세부 점수]** 내용: {breakdown.get('content')}/3, 구성: {breakdown.get('structure')}/3, 언어: {breakdown.get('grammar')}/2")
                st.info(f"**💡 피드백:**\n{wa.get('feedback', '없음')}")
